...
async def show_leaderboard(ctx):
    ...
    if ctx.guild.icon:
        embed.set_author(name=f"{ctx.guild.name} server", icon_url=ctx.guild.icon.url)
    else:
        embed.set_author(name=f"{ctx.guild.name} server")
    ...
    for i, (u_id, lvl, xp) in enumerate(top_users):
        ...
        description_text += f"`{rank_num}`  •  **{name}**  •  LVL: {lvl}\n"
...
async def show_info(ctx):
    ...
    embed.add_field(
        ...
        value="`.rank` — Cek level & XP kamu\n"
         "`.lb` — Top 5 member teraktif (Arcane Style)\n"
